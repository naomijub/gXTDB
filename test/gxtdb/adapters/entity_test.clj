...
(deftest entity-tx->proto-test
  (testing "entity tx response map correctly parses to proto response"
    (is (=  (:xt-id expected) (-> actual entity-tx->proto :xt-id)))
    (is (=  (:content-hash expected) (-> actual entity-tx->proto :content-hash)))
    (is (=  (:tx-id expected) (-> actual entity-tx->proto :tx-id)))))